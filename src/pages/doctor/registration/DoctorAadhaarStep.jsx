...
export const DoctorAadhaarStep = () => {
  const navigate = useNavigate();
  const { docRegState, setAadhaarVerified } = useDoctorRegistration();

  const [aadhaarNumber, setAadhaarNumber] = useState(docRegState.aadhaarNumber || '');
  ...
  const handleSendOTP = async (e) => {
    ...
    try {
      await doctorAuthService.sendAadhaarOTP(aadhaarNumber);
      setOtpSent(true);
      setTimer(30);
      setCanResend(false);
      setInfoMessage('Mock OTP sent to linked mobile number.');
    } catch (err) {
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      await doctorAuthService.verifyAadhaarOTP(aadhaarNumber, otp);
      setSuccessMessage('Identity verified successfully.');
      setAadhaarVerified(aadhaarNumber);

      setTimeout(() => {
        navigate('/doctor/register/medical-verification');
      }, 600);
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#F7F3EA] flex flex-col items-center justify-center py-12 px-4">
      <div className="w-full max-w-sm">
        <DoctorRegistrationHeader
          ...
          subtitle="Verify practitioner personal identity as the first step of doctor onboarding."
        />
        ...
        <div className="bg-[#FAF7F2] border border-[#E5DDD0] rounded-2xl shadow-sm overflow-hidden p-6 space-y-4">
          ...
          <div className="p-3 bg-[#F4EFE6] border border-[#E5DDD0] rounded-lg text-[11px] text-[#787469] flex items-start gap-2">
            ...
            <div>
              <span className="font-semibold text-[#2F2D29]">Prototype Mock Verification: </span>
              This step simulates UIDAI identity proofing for evaluation. No actual Aadhaar data is stored or connected to government servers.
            </div>
          </div>
          ...
          <div className="space-y-4">
            ...
            <form onSubmit={handleVerifyOTP} className="space-y-4 pt-2 border-t border-[#E5DDD0]">
              ...
              <Button
                ...
                icon={ArrowRight}
              >
                {verifying ? 'Verifying Identity…' : 'Verify OTP & Continue'}
              </Button>
            </form>
          </div>
        </div>
